...
<#
    .SYNOPSIS
    Grants the given TeamsCallHoldPolicy
    
    .DESCRIPTION
    Grants the given TeamsCallHoldPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsCallHoldPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsCallHoldPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsCallHoldPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsCallHoldPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsCallHoldPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsCallHoldPolicy @ArgHash
'@)
