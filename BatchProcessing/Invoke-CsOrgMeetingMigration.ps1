...
$FilterScript = [ScriptBlock]::Create('TeamsUpgradePolicy -eq $null -and Enabled -eq $true')
# HACK: WhatIf currently not working for Start-CsExMeetingMigration, so running test against different session instead
$JobScript = if ($IsTest) {
    [ScriptBlock]::Create(@'
param($filterSB, $IsTest)
Get-CsOnlineUser -Filter $filterSB | Grant-CsClientPolicy -PolicyName $null -WhatIf
'@) 
}
else {
    [ScriptBlock]::Create(@'
...
Start-CsExMeetingMigration -SourceMeetingType SfB -TargetMeetingType Teams
'@) 
}
