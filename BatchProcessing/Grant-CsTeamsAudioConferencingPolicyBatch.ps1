...
<#
    .SYNOPSIS
    Grants the given TeamsAudioConferencingPolicy
    
    .DESCRIPTION
    Grants the given TeamsAudioConferencingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsAudioConferencingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsAudioConferencingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsAudioConferencingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsAudioConferencingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsAudioConferencingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsAudioConferencingPolicy @ArgHash
'@)
